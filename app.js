...
function createEarthquakeIcon(magnitude) {
  const size = Math.max(magnitude * 5, 20);
...
// -------------------------------------------------------------
// 4. API Fetchers (Strictly Past 24 Hours)
// -------------------------------------------------------------

// A. Fetch USGS Earthquakes (Past 24 Hours, M4.5+)
  ...
  try {
    ...
    L.geoJSON(data, {
      pointToLayer: (feature, latlng) => {
        ...
        marker.bindPopup(`
          <strong>🔴 Earthquake (Past 24h)</strong><br/>
...
// B. Fetch NASA EONET Active Wildfires (Past 24 Hours Only)
async function loadWildfires() {
  const url = 'https://eonet.gsfc.nasa.gov/api/v3/events?category=wildfires&status=open&days=1';
  try {
    const res = await fetch(url);
    const data = await res.json();
    ...
    data.events.forEach(event => {
      ...
      if (geometry && geometry.type === 'Point') {
        ...
        marker.bindPopup(`
          <strong>🔥 Active Wildfire (Past 24h)</strong><br/>
...
// C. Fetch Volcano Activity (Past 24 Hours Only)
async function loadVolcanoes() {
  const url = 'https://eonet.gsfc.nasa.gov/api/v3/events?category=volcanoes&status=open&days=1';
  try {
    const res = await fetch(url);
    const data = await res.json();
    ...
    data.events.forEach(event => {
      ...
      if (geometry && geometry.type === 'Point') {
        ...
        marker.bindPopup(`
          <strong>🌋 Volcanic Activity (Past 24h)</strong><br/>
          <strong>Name:</strong> ${event.title}<br/>
          <strong>Source:</strong> NASA EONET
        `);
